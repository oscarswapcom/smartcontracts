...
//     function pendingMonk(uint256 _pid, address _user)
//         external
//         view
//         returns (uint256);
...
// /// @notice The (older) MasterChef contract gives out a constant number of MONK tokens per block.
// /// It is the only address with minting rights for MONK.
...
//     ///
//     /// We do some fancy math here. Basically, any point in time, the amount of MONKs
//     /// entitled to a user but is pending to be distributed is:
//     ///
//     ///   pending reward = (user share * pool.accMonkPerShare) - user.rewardDebt
//     ///
//     ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
//     ///   1. The pool's `accMonkPerShare` (and `lastRewardBlock`) gets updated.
...
//     ///     MONK rewards each pool gets.
//     /// `accMonkPerShare` Accumulated MONKs per share, times 1e12.
...
//     ///     the MONK rewards to all the OscarSwap products.
//     /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
//     struct PoolInfo {
//         uint256 accMonkPerShare;
...
//     /// @notice Address of MONK contract.
//     IBEP20 public immutable MONK;

//     /// @notice The only address can withdraw all the burn MONK.
...
//     ///  @notice 40 monks per block in MCV1
//     uint256 public constant MASTERCHEF_MONK_PER_BLOCK = 40 * 1e18;
//     uint256 public constant ACC_MONK_PRECISION = 1e18;

//     /// @notice Basic boost factor, none boosted user's boost factor
//     uint256 public constant BOOST_PRECISION = 100 * 1e10;
//     /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
//     uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
//     /// @notice total monk rate = toBurn + toRegular + toSpecial
//     uint256 public immutable MONK_RATE_TOTAL_PRECISION = 1e12;
//     /// @notice The last block number of MONK burn action being executed.
//     /// @notice MONK distribute % for burn
//     uint256 public monkRateToBurn = 75 * 1e10;
//     /// @notice MONK distribute % for regular farm pool
//     uint256 public monkRateToRegularFarm = 10 * 1e10;
//     /// @notice MONK distribute % for special pools
//     uint256 public monkRateToSpecialFarm = 15 * 1e10;
...
//     /// @param _MASTER_CHEF The OscarSwap MCV1 contract address.
//     /// @param _MONK The MONKINI token contract address.
...
//     /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of MONK.
...
//         // MCV2 start to earn MONK reward from current block in MCV1 pool
...
//     /// only for MONK distributions within OscarSwap products.
...
//         // stake MONK token will cause staked token and reward token mixed up,
//         // may cause staked tokens withdraw as reward token,never do it.
//         require(_lpToken != MONK, "MONK token can't be added to farm pools");
...
//     /// @notice Update the given pool's MONK allocation point. Can only be called by the owner.
...
//     /// @notice View function for checking pending MONK rewards.
//     /// @param _pid The id of the pool. See `poolInfo`.
...
//         uint256 accMonkPerShare = pool.accMonkPerShare;
//         uint256 lpSupply = pool.totalBoostedShare;

//         if (block.number > pool.lastRewardBlock && lpSupply != 0) {
//             uint256 multiplier = block.number.sub(pool.lastRewardBlock);

//             uint256 monkReward = multiplier
//                 .mul(monkPerBlock(pool.isRegular))
//                 .mul(pool.allocPoint)
//                 .div(
//                     (
//                         pool.isRegular
...
//                     )
//                 );
//             accMonkPerShare = accMonkPerShare.add(
//                 monkReward.mul(ACC_MONK_PRECISION).div(lpSupply)
//             );
//         }
...
//             .div(BOOST_PRECISION);
//         return
//             boostedAmount.mul(accMonkPerShare).div(ACC_MONK_PRECISION).sub(
//                 user.rewardDebt
//             );
//     }

//     /// @notice Update monk reward for all the active pools. Be careful of gas spending!
...
//     /// @notice Calculates and returns the `amount` of MONK per block.
//     /// @param _isRegular If the pool belongs to regular or special.
//     function monkPerBlock(bool _isRegular)
//         public
//         view
//         returns (uint256 amount)
//     {
//         if (_isRegular) {
//             amount = MASTERCHEF_MONK_PER_BLOCK.mul(monkRateToRegularFarm).div(
//                 MONK_RATE_TOTAL_PRECISION
//             );
//         } else {
//             amount = MASTERCHEF_MONK_PER_BLOCK.mul(monkRateToSpecialFarm).div(
//                 MONK_RATE_TOTAL_PRECISION
//             );
//         }
//     }

//     /// @notice Calculates and returns the `amount` of MONK per block to burn.
//     function monkPerBlockToBurn() public view returns (uint256 amount) {
//         amount = MASTERCHEF_MONK_PER_BLOCK.mul(monkRateToBurn).div(
//             MONK_RATE_TOTAL_PRECISION
//         );
//     }
...
//                     .mul(pool.allocPoint)
//                     .div(totalAllocPoint);
//                 pool.accMonkPerShare = pool.accMonkPerShare.add(
//                     (monkReward.mul(ACC_MONK_PRECISION).div(lpSupply))
//                 );
//             }
...
//         if (user.amount > 0) {
//             settlePendingMonk(msg.sender, _pid, multiplier);
//         }
...
//             .mul(multiplier)
//             .div(BOOST_PRECISION)
//             .mul(pool.accMonkPerShare)
//             .div(ACC_MONK_PRECISION);
//         poolInfo[_pid] = pool;
...
//     /// @notice Harvests MONK from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
...
//     /// @notice Send MONK pending for burn to `burnAdmin`.
//     /// @param _withUpdate Whether call "massUpdatePools" operation.
//     function burnMonk(bool _withUpdate) public onlyOwner {
//         if (_withUpdate) {
//             massUpdatePools();
//         }

//         uint256 multiplier = block.number.sub(lastBurnedBlock);
//         uint256 pendingMonkToBurn = multiplier.mul(monkPerBlockToBurn());

//         // SafeTransfer MONK
//         _safeTransfer(burnAdmin, pendingMonkToBurn);
//         lastBurnedBlock = block.number;
//     }

//     /// @notice Update the % of MONK distributions for burn, regular pools and special pools.
//     /// @param _burnRate The % of MONK to burn each block.
//     /// @param _regularFarmRate The % of MONK to regular pools each block.
//     /// @param _specialFarmRate The % of MONK to special pools each block.
//     /// @param _withUpdate Whether call "massUpdatePools" operation.
//     function updateMonkRate(
//         uint256 _burnRate,
...
//             "MasterChefV2: Monk rate must be greater than 0"
//         );
//         require(
...
//             massUpdatePools();
//         }
//         // burn monk base on old burn monk rate
//         burnMonk(false);

//         monkRateToBurn = _burnRate;
//         monkRateToRegularFarm = _regularFarmRate;
//         monkRateToSpecialFarm = _specialFarmRate;

//         emit UpdateMonkRate(_burnRate, _regularFarmRate, _specialFarmRate);
//     }
...
//         settlePendingMonk(_user, _pid, prevMultiplier);
...
//     /// @notice Settles, distribute the pending MONK rewards for given user.
//     /// @param _user The user address for settling rewards.
//     /// @param _pid The pool id.
//     /// @param _boostMultiplier The user boost multiplier in specific pool id.
//     function settlePendingMonk(
//         address _user,
//         uint256 _pid,
...
//             BOOST_PRECISION
//         );
//         uint256 accMonk = boostedAmount.mul(poolInfo[_pid].accMonkPerShare).div(
//             ACC_MONK_PRECISION
//         );
//         uint256 pending = accMonk.sub(user.rewardDebt);
//         // SafeTransfer MONK
//         _safeTransfer(_user, pending);
//     }

//     /// @notice Safe Transfer MONK.
//     /// @param _to The MONK receiver address.
//     /// @param _amount transfer MONK amounts.
//     function _safeTransfer(address _to, uint256 _amount) internal {
//         if (_amount > 0) {
//             // Check whether MCV2 has enough MONK. If not, harvest from MCV1.
//             if (MONK.balanceOf(address(this)) < _amount) {
//                 harvestFromMasterChef();
//             }
//             uint256 balance = MONK.balanceOf(address(this));
//             if (balance < _amount) {
//                 _amount = balance;
//             }
//             MONK.safeTransfer(_to, _amount);
//         }
//     }
// }
